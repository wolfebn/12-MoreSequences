...
def make_less_simple_string(m, n):
    ...
    lesssimpleS = ''
    for k in range(n - m + 1):
        lesssimpleS = lesssimpleS + str(m + k)
        if k < (n - m):
            lesssimpleS = lesssimpleS + '-'
...
def rectangles_from_circles(circles):
    ...
    for k in range(len(circles)):
        p1 = rg.Point(0,0)
        p2 = rg.Point(0,0)
        p1.x = circles[k].center.x - circles[k].radius
        p1.y = circles[k].center.y + circles[k].radius
        p2.x = circles[k].center.x + circles[k].radius
        p2.y = circles[k].center.y - circles[k].radius
        outline = rg.Rectangle(p1, p2)
        rectangles = rectangles + [outline]
    return rectangles
